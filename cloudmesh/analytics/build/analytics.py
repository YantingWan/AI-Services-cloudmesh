...
def save_obj(obj, class_name):
    np.save(os.path.join('d:\iublife\2019fall\cloudm\cloudmesh-analytics\cloudmesh\analytics\build', str(class_name) + '_constructor'),
...
def LinearRegression_fit(body):
    ...
    try:
        obj =load_obj(os.path.join('d:\iublife\2019fall\cloudm\cloudmesh-analytics\cloudmesh\analytics\build', 'LinearRegression_constructor.npy'))
...
def LinearRegression_get_properties(body):
    if 'X' in body['paras'].keys() and isinstance(body['paras']['X'], str):
        file_name = body['paras']['X']
        body['paras']['X'] = file.read_csv(file_name)
    ...
    try:
        obj =load_obj(os.path.join('d:\iublife\2019fall\cloudm\cloudmesh-analytics\cloudmesh\analytics\build', 'LinearRegression_constructor.npy'))
        res = obj.get_properties(**body['paras'])
        save_obj(obj, 'LinearRegression')
    except Exception as e:
        return jsonify({'Error': str(e)})
